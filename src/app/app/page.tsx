...
import { buildSectorPEMap, healthTier } from "@/lib/signals";
    ...
    <>
      ...
      {plan === "free" ? (
        <div className="mk-banner">
          <div className="mk-banner-in">
            <span style={{ fontSize: 18 }}>🔓</span>
            <div><b>Free plan</b> — you can see the risky 🔴 and mediocre 🟡 names. The top-rated 🟢 winners are locked.</div>
            <button className="mk-btn mk-btn-primary" style={{ marginLeft: "auto", padding: "7px 16px", fontSize: 13, whiteSpace: "nowrap" }} onClick={() => setShowUpgrade(true)}>Unlock all — $12.99/mo</button>
          </div>
        </div>
      ) : (
        <div className="mk-banner">
          <div className="mk-banner-in" style={{ borderColor: "rgba(52,211,153,.3)", background: "linear-gradient(90deg,rgba(52,211,153,.12),rgba(34,211,238,.06))" }}>
            <span style={{ fontSize: 18 }}>✓</span>
            <div><b style={{ color: "var(--emerald)" }}>Pro</b> — full access unlocked. All stocks, alerts, and AI insights.</div>
          </div>
        </div>
      )}
          ...
          <>
            ...
            {activeTab === "sectors" ? (
              ...
              <ResultsTable
                ...
                isLocked={plan === "free" ? (r) => healthTier(r).level === "good" : undefined}
      ...
      {showUpgrade && (
        <div className="up-modal-bg" onClick={(e) => { if (e.target === e.currentTarget) setShowUpgrade(false); }}>
          <div className="glass up-modal">
            <div className="up-promo">🚀 Launch offer · 40% off first month</div>
            <div className="up-ic">⚡</div>
            <h3>Unlock the winners</h3>
            <p>The top-rated 🟢 stocks, every scan, smart alerts, portfolio health and AI insights — <b>everything</b>, one price.</p>
            <div className="up-price"><span className="up-was num">$12.99</span><span className="num">$7.79</span><span className="up-per">first month</span></div>
            <button className="mk-btn mk-btn-primary" style={{ width: "100%", padding: 13, fontSize: 16, marginBottom: 10 }}
              onClick={() => { setPlan("pro"); setShowUpgrade(false); }}>
              Claim 40% off (Stripe)
            </button>
            <button className="mk-btn mk-btn-ghost" style={{ width: "100%" }} onClick={() => setShowUpgrade(false)}>Maybe later</button>
            <p className="up-fine">Then $12.99/mo · 14-day free trial · cancel anytime · demo flips you to Pro</p>
          </div>
        </div>
      )}
    </>
